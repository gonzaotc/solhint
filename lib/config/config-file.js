...
// `require(esm)` returns a module namespace, so an ESM config arrives as
// `{ __esModule, default }` instead of the config object. Unwrap it, as loadPlugin does.
const interopDefault = (config) =>
...
const loadConfig = (configFile) => {
  ...
  if (configFile) {
    return interopDefault(explorer.load(configFile).config)
  }
  ...
  return interopDefault(searchedFor.config) || createEmptyConfig()
}
...
const configGetter = (p) => {
  ...
  return interopDefault(requireFromProject(resolved))
}
